...
public class CustomStructType implements UaStructure {
	...
	public static final ExpandedNodeId TYPE_ID = ExpandedNodeId
			.parse(String.format("nsu=%s;s=%s", ExampleNamespace.NAMESPACE_URI, "DataType.CustomStructType"));

	public static final ExpandedNodeId BINARY_ENCODING_ID = ExpandedNodeId
			.parse(String.format("nsu=%s;s=%s", ExampleNamespace.NAMESPACE_URI, "DataType.CustomStructType.BinaryEncoding"));
	...
	public boolean equals(Object o) {
		...
		return baz == that.baz && Objects.equal(foo, that.foo) && Objects.equal(bar, that.bar) && customEnumType == that.customEnumType;
	}
	...
	public String toString() {
		return MoreObjects.toStringHelper(this).add("foo", foo).add("bar", bar).add("baz", baz).add("customEnumType", customEnumType)
				.toString();
	}
