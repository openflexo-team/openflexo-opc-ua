	...
	private OpcUaClient createClient() throws Exception {
		...
		return OpcUaClient.create(clientExample.getEndpointUrl(),
				...
				configBuilder -> configBuilder.setApplicationName(LocalizedText.english("eclipse milo opc-ua client"))
						...
						.setCertificate(loader.getClientCertificate()).setCertificateChain(loader.getClientCertificateChain())
						.setCertificateValidator(certificateValidator).setIdentityProvider(clientExample.getIdentityProvider())
		...
		try {
			...
			if (serverRequired) {
				...
				client.getConfig().getCertificate().ifPresent(
						certificate -> exampleServer.getServer().getConfig().getTrustListManager().addTrustedCertificate(certificate));
				...
				exampleServer.getServer().getConfig().getCertificateManager().getCertificates()
						.forEach(certificate -> trustListManager.addTrustedCertificate(certificate));
			}
